	...
	Calendar.prototype = {
		...
		_initializeOptions: function(opt) {
			...
			this.options = {
				...
				language : (opt.language != null && dates[opt.language] != null) ? opt.language : 'cn',
				...
				while(currentDate <= lastDate)
				{
					...
					do
					{
						...
						if(currentDate < firstDate) {
							...
							cellContent.attr({
								"href" : (currentDate.getFullYear().toString()).slice(-2) + ("0" + (currentDate.getMonth() + 1)).slice(-2) + ("0" + currentDate.getDate()).slice(-2) + "/html/awstats.default.html",
								"target" : "_blank"
							});
